    ...
    try {
        ...
        if (command) {
            if (command.name === 'join') {
                if (joincd.size === 0) {
                    joincd.set('join', Date.now());
                    setTimeout(() => joincd.delete('join'), 500);
                } else {
                    const expirationTime = Date.now() - joincd.get('join'); //get the amount of time he needs to wait until he can run the cmd again
            ...
            try {
                ...
                if (command.memberpermissions && !message.member.hasPermission(command.memberpermissions, { checkAdmin: command.adminPermOverride, checkOwner: command.adminPermOverride })) {
                    ...
                    ).then(msg => msg.delete({ timeout: 3000 }).catch(e => console.log("Couldn't Delete --> Ignore".gray)));
                }
